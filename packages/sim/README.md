...
- **A fixed OB boundary independent of the corridor** (`terrain.ts#lieAt`'s land-mode branch, gated on `TerrainQuery.landEnvelope`): `lieAt`'s in-bounds test is symmetric about whichever centerline it's given, so once the centerline bends to route around a hazard there is no per-station `obHalfWidth` that can hold a *fixed* boundary still — matching a fixed band `[-W, W]` at a drifted station algebraically forces the drift to be zero. Provably impossible in the one-polyline corridor model, not just imprecise. So for a land parcel, `lieAt` checks the point against `landEnvelope`'s own rectangle directly instead — a boundary the router's bend can't distort. It's also intentionally forgiving: real OB (stroke and distance) only starts `LAND_FRINGE_YARDS` (14, matching `apps/web`'s render margin so it never appears inside the visible frame) past the authored rectangle; the ring just inside that is filled with fixed `deep`-lie terrain (`fairway.ts#fringeBands` — rocks/scrub, playable but punishing) rather than a hard OB wall, and anywhere past the routed corridor's own arc-length ends but still on the land resolves plain `rough`, never OB. A hand-authored (non-land-mode) corridor is unaffected — `lieAt` only takes this branch when `landEnvelope` is present, so tray-mode's OB semantics (arc-length-beyond and lateral `obHalfWidth`, both immediate OB) are exactly as before.
...
- **Tapered/non-rectangular land envelopes** — `LandEnvelope` is a constant-width rectangle; both `lieAt`'s land-mode OB rectangle test and `deriveFairway`'s fringe-band construction (two fixed lateral `deep`-lie bands) assume that shape specifically and would need reworking for a land parcel whose width varies along its length.
...
- `fairway.ts`'s derived-corridor shape numbers — `teeGapLong`/`teeGapPar3` (how far short of the tee/green the mown fairway starts), `greenApron` (how far past the green the rounded cap reaches), `CAP_PROFILE`'s 6-station quarter/semicircle sampling, and the tee-box's own dimensions — are all eyeballed for visual plausibility, not calibrated against a real hole or playtested for difficulty.
